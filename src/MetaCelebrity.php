 ...
 */
class MetaCelebrity extends Model
{
    /**
     ...
     * @var string
     */
    protected $pk = 'celebrity_id';

    /**
     * Api列表的查询字段
     */
    const ApiListsField = ['celebrity_id', 'name', 'aka', 'gender', 'alt', 'avatars', 'state', 'imdb_nm'];
     ...
     */
    public static function getCelebrity(array|string $ids, array $field = self::ApiListsField): array
    {
        if (empty($ids)) {
            return [];
        }
        ...
        $list = self::whereIn('celebrity_id', $ids)->field($field)->select();
        if ($list->isEmpty()) {
            return [];
        }
        return $list->toArray();
     ...
     * @return string
     */
    public function getNameAttr($value): string
    {
        return MetaName::getNameText($value);
    }
    ...
    {
        return MetaName::getNameArray($value);
    }
}
